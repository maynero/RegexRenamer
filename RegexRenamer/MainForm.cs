  ...
  public partial class MainForm : Form
  {
      ...
      try
      {
        ...
        using( RegistryKey key = Registry.CurrentUser.OpenSubKey( "Software\\RegexRenamer\\Options" ) )
        {
          if( key != null )
          {
            ...
            itmOptionsIncludeSubfolders.Checked = (string)key.GetValue("IncludeSubfolders") == "True";
      ...
      try
      {
        ...
        using( RegistryKey key = Registry.CurrentUser.CreateSubKey( "Software\\RegexRenamer\\Options" ) )
        {
          if( key != null )
          {
            ...
            key.SetValue( "IncludeSubfolders", itmOptionsIncludeSubfolders.Checked );
            key.Close();
          }
        }
    ...
    private void UpdateFileList()
    {
      ...
      else // files
      {
        ...
        try
        {
          if (itmOptionsIncludeSubfolders.Checked) {
    ...
    private void itmOptionsIncludeSubfolders_Click(object sender, EventArgs e) {
        this.Update();
        UpdateFileList();
    }
    ...
    private void bgwRename_DoWork( object sender, DoWorkEventArgs e )
    {
      ...
      for( int afi = 0; afi < activeFiles.Count; afi++ )
      {
        ...
        string newFullpath = "";
        if (itmOutputRenameInPlace.Checked) {
            newFullpath = Path.Combine(Path.GetDirectoryName(activeFiles[afi].Fullpath), activeFiles[afi].Preview);
        }
        else {
            newFullpath = Path.Combine( outputPath, activeFiles[afi].Preview );
        }
        if( itmOptionsPreserveExt.Checked )
